...
void handle_external_n_entry_duplication(symbol_address *entries, symbol_address *symbol_table, int entry_amount, int extern_amount, error *error, int label_amount);

void second_pass(word_data *commands, word_data *instructions, symbol_address *entries, symbol_address *symbol_table, error *error, int IC, int DC, int label_amount, int entry_amount, int extern_amount) {
...
void handle_duplication(symbol_address *symbol_table, int label_amount, error *error) {
	...
	for (current_symbol = 0; current_symbol < label_amount-1; current_symbol++) {
		found = search_symbol_by_name(symbol_table[current_symbol].symbol_name, &symbol_table[current_symbol+1], label_amount - current_symbol - 1);
		if (found != -1) {
			...
			print_two_line_error(DUPLICATE_SYMBOL_MESSAGE, SYMBOL_ALREADY_EXISTS_DESCRIPTION, line2, line, symbol_table[current_symbol].line_number, symbol_table[found].line_number, strstr(line2, symbol_table[current_symbol].symbol_name) - line2 + 1, strstr(line2, symbol_table[current_symbol].symbol_name) - line2 + strlen(symbol_table[current_symbol].symbol_name) + 1, strstr(line, symbol_table[found].symbol_name) - line + 1, strstr(line, symbol_table[found].symbol_name) - line + strlen(symbol_table[found].symbol_name) + 1, error);
		}
	}
}
...
int set_symbol_address(symbol_address *symbol_table, int label_amount, int IC, word_data *commands, error *error) {
	...
	for (word_number = 0; word_number < IC; word_number++) {
		...
		if (found == -1) {
			...
			print_general_error(SYMBOL_NOT_FOUND_MESSAGE, SYMBOL_NOT_FOUND_DESCRIPTION, line, commands[word_number].line_number, strstr(line, commands[word_number].symbol) - line + 1, strstr(line, commands[word_number].symbol) - line + strlen(commands[word_number].symbol) + 1, 0, 0, error);
	...
	for (i = 0; i < label_amount; i++) {
		...
		if (found != -1) {
			...
			if (symbol_table[i].line_number > entries[found].line_number)
				print_two_line_error(DUPLICATE_SYMBOL_MESSAGE, SYMBOL_ALREADY_EXISTS_DESCRIPTION, line2, line, entries[found].line_number, symbol_table[i].line_number, strstr(line2, symbol_table[i].symbol_name) - line2 + 1, strstr(line2, symbol_table[i].symbol_name) - line2 + strlen(symbol_table[i].symbol_name) + 1, strstr(line, entries[found].symbol_name) - line + 1, strstr(line, entries[found].symbol_name) - line + strlen(entries[found].symbol_name) + 1, error);
			else
				print_two_line_error(DUPLICATE_SYMBOL_MESSAGE, SYMBOL_ALREADY_EXISTS_DESCRIPTION, line, line2, symbol_table[i].line_number, entries[found].line_number, strstr(line, entries[found].symbol_name) - line + 1, strstr(line, entries[found].symbol_name) - line + strlen(entries[found].symbol_name) + 1, strstr(line2, symbol_table[i].symbol_name) - line2 + 1, strstr(line2, symbol_table[i].symbol_name) - line2 + strlen(symbol_table[i].symbol_name) + 1, error);
